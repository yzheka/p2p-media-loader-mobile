...
const y = j(function(s, t, e) {
  ...
  try {
    ...
    return i.prototype = Object.create(Object.getPrototypeOf(s)), Re(new i(), e);
  }
...
for (var Is = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", Rs = typeof Uint8Array > "u" ? [] : new Uint8Array(256), at = 0; at < 64; at++) Rs[Is.charCodeAt(at)] = at;
...
const T = A("bittorrent-tracker:websocket-tracker"), R = {};
class Xt extends Os {
  constructor(t, e) {
  ...
  destroy(t = Pe) {
    ...
    if (this.peers = null, this.socket && (this.socket.removeListener("connect", this._onSocketConnectBound), this.socket.removeListener("data", this._onSocketDataBound), this.socket.removeListener("close", this._onSocketCloseBound), this.socket.removeListener("error", this._onSocketErrorBound), this.socket = null), this._onSocketConnectBound = null, this._onSocketErrorBound = null, this._onSocketDataBound = null, this._onSocketCloseBound = null, R[this.announceUrl] && (R[this.announceUrl].consumers -= 1), R[this.announceUrl].consumers > 0) return t();
    let e, n = R[this.announceUrl];
    if (delete R[this.announceUrl], n.on("error", Pe), n.once("close", t), !this.expectingResponse) return i();
    function i() {
  ...
  _openSocket() {
    if (this.destroyed = !1, this.peers || (this.peers = {}), this._onSocketConnectBound = () => {
      ...
      this._onSocketClose();
    }, this.socket = R[this.announceUrl], this.socket) R[this.announceUrl].consumers += 1, this.socket.connected && this._onSocketConnectBound();
    else {
      const t = new URL(this.announceUrl);
      let e;
      this.client._proxyOpts && (e = t.protocol === "wss:" ? this.client._proxyOpts.httpsAgent : this.client._proxyOpts.httpAgent, !e && this.client._proxyOpts.socksProxy && (e = this.client._proxyOpts.socksProxy)), this.socket = R[this.announceUrl] = new kn({ url: this.announceUrl, agent: e }), this.socket.consumers = 1, this.socket.once("connect", this._onSocketConnectBound);
    }
...
Xt.prototype.DEFAULT_ANNOUNCE_INTERVAL = 3e4, Xt._socketPool = R;
...
const Y = Rt("cstr", 4), st = Rt("cend", 4), se = Rt("dstr", 4), ie = Rt("dend", 4), Qs = [Y, se], zs = [st, ie], Fe = Y.length + st.length;
function In(s) {
...
const Js = Object.freeze(Object.defineProperty({ __proto__: null, BinaryCommandChunksJoiner: Rn, BinaryCommandJoiningError: xt, PeerCommandType: N, deserializeCommand: En, isCommandChunk: In, serializePeerCommand: qn }, Symbol.toStringTag, { value: "Module" }));
class Vs {
  constructor(t, e, n, i) {
    ...
    d(this, "onCommandReceived", async (t) => {
      ...
      switch (t.c) {
        ...
        case k.SegmentDataSendingCompleted: {
          ...
          if (!a.data || o.request.segment.externalId !== t.i || o.requestId !== t.r) return a.clearLoadedBytes(), this.cancelSegmentDownloading("peer-protocol-violation"), void this.destroy();
...
class ii {
  ...
  resolve(t, e) {
    this._status === "pending" && (console.log(">> resolve request: ", this.segment.externalId), this._status = "succeed", this.engineCallbacks.onSuccess({ data: t, bandwidth: e }));
  }
  reject() {
    this._status === "pending" && (console.log(">> reject request: ", this.segment.externalId), this._status = "failed", this.engineCallbacks.onError(new ae("failed")));
  }
  abort() {
    this._status === "pending" && (console.log(">> abort request:", this.segment.externalId), this._status = "aborted", this.engineCallbacks.onError(new ae("aborted")));
  }
...
class ri {
  ...
  async loadSegment(t, e) {
    ...
    try {
      ...
      this.engineRequest && this.engineRequest.abort(), this.engineRequest = o;
    } catch {
      o.reject();
    } finally {
...
const U = class U {
  ...
  async loadSegment(t, e) {
    ...
    console.log(">> new request", n.externalId), this.getStreamHybridLoader(n).loadSegment(n, e);
  }
