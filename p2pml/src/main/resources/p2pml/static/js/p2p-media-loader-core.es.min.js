...
var Bn = (s, t, e) => t in s ? Pn(s, t, { enumerable: !0, configurable: !0, writable: !0, value: e }) : s[t] = e;
...
var E, q, We = { exports: {} }, _ = We.exports = {};
function Ut() {
...
function Qe(s) {
  if (E === setTimeout) return setTimeout(s, 0);
  if ((E === Ut || !E) && setTimeout) return E = setTimeout, setTimeout(s, 0);
  try {
    return E(s, 0);
  } catch {
    try {
...
(function() {
  try {
    E = typeof setTimeout == "function" ? setTimeout : Ut;
  } catch {
    E = Ut;
  }
  try {
    q = typeof clearTimeout == "function" ? clearTimeout : Ht;
  } catch {
    q = Ht;
  }
})();
var H, P = [], Q = !1, pt = -1;
function Mn() {
  Q && H && (Q = !1, H.length ? P = H.concat(P) : pt = -1, P.length && ze());
}
function ze() {
  if (!Q) {
    var s = Qe(Mn);
    ...
    for (var t = P.length; t; ) {
      for (H = P, P = []; ++pt < t; ) H && H[pt].run();
      pt = -1, t = P.length;
    }
    H = null, Q = !1, function(e) {
      if (q === clearTimeout) return clearTimeout(e);
      if ((q === Ht || !q) && clearTimeout) return q = clearTimeout, clearTimeout(e);
      try {
        return q(e);
      } catch {
        try {
...
_.nextTick = function(s) {
  ...
  P.push(new he(s, t)), P.length !== 1 || Q || Qe(ze);
}, he.prototype.run = function() {
  this.fun.apply(null, this.array);
}, _.title = "browser", _.browser = !0, _.env = {}, _.argv = [], _.version = "", _.versions = {}, _.on = R, _.addListener = R, _.once = R, _.off = R, _.removeListener = R, _.removeAllListeners = R, _.emit = R, _.prependListener = R, _.prependOnceListener = R, _.listeners = function(s) {
  return [];
...
class ps {
  ...
  pipe(t, e) {
    ...
    if (typeof e != "function" && (e = null), this.stream._duplexState |= 512, this.pipeTo = t, this.pipeline = new fs(this.stream, t, e), e && this.stream.on("error", Re), Sn(t)) t._writableState.pipeline = this.pipeline, e && t.on("error", Re), t.on("finish", this.pipeline.finished.bind(this.pipeline));
    else {
  ...
  push(t) {
    const e = this.stream;
    return t === null ? (this.highWaterMark = 0, e._duplexState = 536805311 & e._duplexState | 1024, !1) : this.map !== null && (t = this.map(t)) === null ? (e._duplexState &= ke, this.buffered < this.highWaterMark) : (this.buffered += this.byteLength(t), this.queue.push(t), e._duplexState = (e._duplexState | qt) & ke, this.buffered < this.highWaterMark);
  }
  shift() {
    const t = this.queue.shift();
  ...
  drain() {
    const t = this.stream;
    for (; (16527 & t._duplexState) === qt && 768 & t._duplexState; ) {
      const e = this.shift();
  ...
  continueUpdate() {
    return !!(this.stream._duplexState & Pt) && (this.stream._duplexState &= an, !0);
  }
  updateCallback() {
    (32879 & this.stream._duplexState) === rn ? this.update() : this.updateNextTick();
  }
  updateNextTick() {
    this.stream._duplexState & Pt || (this.stream._duplexState |= Pt, 32 & this.stream._duplexState || en(this.afterUpdateNextTick));
  }
}
class fs {
  constructor(t, e, n) {
...
const y = j(function(s, t, e) {
  ...
  try {
    ...
    return i.prototype = Object.create(Object.getPrototypeOf(s)), Ie(new i(), e);
  }
...
for (var Rs = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", Is = typeof Uint8Array > "u" ? [] : new Uint8Array(256), at = 0; at < 64; at++) Is[Rs.charCodeAt(at)] = at;
const Es = new TextDecoder(), vn = (s, t) => Es.decode(s), qs = new TextEncoder(), oe = (s) => qs.encode(s), W = (s) => {
  let t, e = "", n = 0;
  const i = s.length;
  ...
  return e;
}, Ee = typeof window < "u" ? window : self, Yt = Ee.crypto || Ee.msCrypto || {};
Yt.subtle || Yt.webkitSubtle;
const Tt = (s) => {
  const t = new Uint8Array(s);
  return Yt.getRandomValues(t);
}, Ps = A("simple-peer"), Ot = 65536;
function qe(s) {
...
let et = class Kt extends bn {
  ...
  _debug() {
    ...
    e[0] = "[" + this._id + "] " + e[0], Ps.apply(null, e);
  }
};
...
const T = A("bittorrent-tracker:websocket-tracker"), I = {};
class Xt extends Os {
  constructor(t, e) {
  ...
  destroy(t = Pe) {
    ...
    if (delete I[this.announceUrl], n.on("error", Pe), n.once("close", t), !this.expectingResponse) return i();
    function i() {
...
}, Rt = Tn.exports = function(s) {
  return Ds(s).toHex();
}, _t = Rt.fromBytes = function(s) {
  for (var t = function(m) {
    for (var f = m.length, v = f << 3, F = new Uint32Array(f + 72 >>> 6 << 4), D = 0, Et = m.length; D < Et; ++D) F[D >>> 2] |= m.charCodeAt(D) << ((3 & D) << 3);
...
var dt = [0, 6, 12, 1, 7, 13, 2, 8, 14, 3, 9, 15, 4, 10, 5, 11], Ms = Rt.salt = function(s) {
  var t = "";
  s || (s = 8);
  do
...
}("1.0.0")}-`, Us = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
function $(s) {
...
const Y = It("cstr", 4), st = It("cend", 4), se = It("dstr", 4), ie = It("dend", 4), Qs = [Y, se], zs = [st, ie], Fe = Y.length + st.length;
function Rn(s) {
...
const Js = Object.freeze(Object.defineProperty({ __proto__: null, BinaryCommandChunksJoiner: In, BinaryCommandJoiningError: xt, PeerCommandType: N, deserializeCommand: En, isCommandChunk: Rn, serializePeerCommand: qn }, Symbol.toStringTag, { value: "Module" }));
class Vs {
  constructor(t, e, n, i) {
    ...
    d(this, "onDataReceived", (t) => {
      Rn(t) ? this.receivingCommandBytes(t) : (this.eventHandlers.onSegmentChunkReceived(t), this.onChunkDownloaded(t.byteLength, "p2p", this.connection.idUtf8));
    });
  ...
  receivingCommandBytes(t) {
    this.commandChunks || (this.commandChunks = new In((e) => {
    ...
    d(this, "onCommandReceived", async (t) => {
      ...
      switch (t.c) {
        ...
        case k.SegmentDataSendingCompleted: {
          ...
          if (o.request.segment.externalId !== t.i || o.requestId !== t.r || !a.data) return a.clearLoadedBytes(), this.cancelSegmentDownloading("peer-protocol-violation"), void this.destroy();
          ...
          const u = await ((n = (e = this.peerConfig).validateP2PSegment) == null ? void 0 : n.call(e, a.segment.url, a.segment.byteRange, a.data)) ?? !0;
...
class ii {
  constructor(t, e) {
    ...
    d(this, "_seed", Math.random());
    this.segment = t, this.engineCallbacks = e, console.log(">>> New engineRequest", this._seed);
  }
  get status() {
    return this._status;
  }
  ...
  resolve(t, e) {
    console.log(`>>>> resolve ${this._seed} = ${t.byteLength}`), this._status === "pending" && (this._status = "succeed", this.engineCallbacks.onSuccess({ data: t, bandwidth: e }));
  }
  reject() {
...
class ri {
  ...
  async loadSegment(t, e) {
    ...
    try {
      ...
      this.engineRequest && (this.engineRequest.abort(), this.engineRequest = void 0), this.engineRequest = o;
    } catch {
      o.reject();
    } finally {
  ...
  processQueue() {
    ...
    console.log("===Queue Length", t.length), e.forEach((h) => {
      console.log("===Queue Segment Ids", h);
  ...
  updatePlayback(t, e) {
    ...
    i && (this.playback.position = t), n && e !== 0 && (this.playback.rate = e), r && (console.log("position significantly changed"), this.logger("position significantly changed"), (o = this.engineRequest) == null || o.markAsShouldBeStartedImmediately()), this.segmentStorage.onPlaybackUpdated(t, e), this.requestProcessQueueMicrotask(r);
  }
  updateStream(t) {
