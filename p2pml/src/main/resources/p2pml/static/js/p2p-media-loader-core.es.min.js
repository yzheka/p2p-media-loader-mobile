...
class HttpRequestExecutor {
  constructor(request, httpConfig, eventTarget) {
    ...
    if (request.loadedBytes !== 0) {
      this.requestByteRange = (_a = this.requestByteRange) != null ? _a : { start: 0 };
    ...
    try {
      ...
      if (!request) {
        const headers = new Headers(
          this.requestByteRange ? {
            Range: `bytes=${this.requestByteRange.start}-${(_c = this.requestByteRange.end) != null ? _c : ""}`
          } : void 0
        );
...
function intToBytes(num) {
  ...
  for (let i = 0; i < bytesAmountNumber; i++) {
    const shift = BigInt(8) * (bytesAmount - BigInt(1) - BigInt(i));
    const byte = num >> shift & BigInt(0xff);
    bytes[i] = Number(byte);
  }
...
function bytesToInt(bytes) {
  ...
  const getNumberPart = (byte, i) => {
    const shift = BigInt(8) * (byteLength - BigInt(1) - BigInt(i));
...
function serializeSimilarIntArray(numbers) {
  ...
  for (const number of numbers) {
    const common2 = number & BigInt(~0xff);
    const diffByte = number & BigInt(0xff);
    const bytes = (_a = commonPartNumbersMap.get(common2)) != null ? _a : new ResizableUint8Array();
  ...
  for (const [commonPart, binaryArray] of commonPartNumbersMap) {
    ...
    const commonPartWithLength = commonPart | BigInt(length) & BigInt(0xff);
...
function deserializeSimilarIntArray(bytes) {
  ...
  for (let i = 0; i < commonPartArraysAmount; i++) {
    ...
    offset += byteLength;
    const arrayLength = commonPartWithLength & BigInt(0xff);
    const commonPart = commonPartWithLength & BigInt(~0xff);
    ...
    __publicField(this, "onCommandReceived", async (command) => {
      ...
      switch (command.c) {
        ...
        case PeerCommandType.SegmentDataSendingCompleted: {
          ...
          const isValid = (_c = await ((_b = (_a = this.peerConfig).validateP2PSegment) == null ? void 0 : _b.call(
            _a,
            request.segment.url,
        ...
        case PeerCommandType.SegmentAbsent:
          if (((_d = this.downloadingContext) == null ? void 0 : _d.request.segment.externalId) === command.i && this.downloadingContext.requestId === command.r) {
...
const _P2PTrackerClient = class _P2PTrackerClient {
  constructor(streamSwarmId, stream, eventHandlers, config, eventTarget) {
    ...
    let peerId = _P2PTrackerClient.PEER_ID_BY_INFO_HASH.get(streamHash);
    if (!peerId) {
      peerId = generatePeerId(config.trackerClientVersionPrefix);
      _P2PTrackerClient.PEER_ID_BY_INFO_HASH.set(streamHash, peerId);
    }
...
peerId: ${peerId}`
  ...
  *peers() {
  ...
  static clearPeerIdCache() {
    _P2PTrackerClient.PEER_ID_BY_INFO_HASH.clear();
  }
};
__publicField(_P2PTrackerClient, "PEER_ID_BY_INFO_HASH", /* @__PURE__ */ new Map());
...
class P2PLoader {
  constructor(streamManifestUrl, stream, requests, segmentStorage, config, eventTarget, onSegmentAnnouncement) {
    ...
    __publicField(this, "onSegmentRequested", async (peer, segmentExternalId, requestId, byteFrom) => {
      ...
      const swarmId = (_a = this.config.swarmId) != null ? _a : this.streamManifestUrl;
...
class HybridLoader {
  ...
  async loadSegment(segment, callbacks) {
    ...
    try {
      ...
      (_b = this.engineRequest) == null ? void 0 : _b.abort();
    ...
    for (const request of this.requests.items()) {
      ...
      switch (status) {
        ...
        case "succeed": {
          ...
          this.requests.remove(request);
          const swarmId = (_b = this.config.swarmId) != null ? _b : this.streamManifestUrl;
          const streamSwarmId = getStreamSwarmId(swarmId, stream);
  ...
  generateQueue() {
    ...
    for (const item of generateQueue(
      ...
      if (this.segmentStorage.hasSegment(
        ...
        streamSwarmId,
        segment.externalId
      ) || ((_b = this.requests.get(segment)) == null ? void 0 : _b.status) === "succeed") {
        alreadyLoadedCount++;
        continue;
      }
      queue2.push(item);
...
class SegmentMemoryStorage {
  ...
  getStreamTimeWindow(streamType, configKey) {
    ...
    return (_a = config == null ? void 0 : config[configKey]) != null ? _a : 0;
  }
  destroy() {
    this.cache.clear();
  }
...
const _Core = class _Core {
  ...
  constructor(config) {
    ...
    const filteredConfig = filterUndefinedProps(config != null ? config : {});
  ...
  getUpdatedStreamProperty(propertyName, updatedConfig, streamType) {
    ...
    return (_a = updatedStreamConfig == null ? void 0 : updatedStreamConfig[propertyName]) != null ? _a : updatedConfig[propertyName];
  }
  /**
  ...
  getStreamHybridLoader(segment) {
    ...
    if (segment.stream.type === "main") {
      (_a = this.mainStreamLoader) != null ? _a : this.mainStreamLoader = this.createNewHybridLoader(segment);
      return this.mainStreamLoader;
    } else {
      (_b = this.secondaryStreamLoader) != null ? _b : this.secondaryStreamLoader = this.createNewHybridLoader(segment);
